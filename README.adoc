...
On the contrary, the trigger that detects ERROR message will fire on a single ERROR message.
 +
These triggers do not AUTORESOLVE the issue, so this means that these applications alerts need human intervetion to be resolved. 
...
Using _name2_ will generate an app ERROR log.
 +
Try to validate how Dampening is working and only and alert will be generated when there are 3 WARN messages per 10 total messages. +
