...
function import_definitions() {
	CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST --header "Hawkular-Tenant: $TENANT" --header "Content-Type:application/json" --data-binary "@$1" http://localhost:8080/hawkular/alerts/import/all)
